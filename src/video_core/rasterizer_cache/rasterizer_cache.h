...
void RasterizerCache<T>::DownloadFillSurface(Surface& surface, SurfaceInterval interval) {
    ...
    ASSERT(surface.fill_size >= 2 && surface.fill_size <= surface.fill_data.size());
    const u32 start_offset = flush_start - surface.addr;
    const std::size_t mapped_size = dest_ptr.GetSize();
    if (start_offset >= mapped_size) [[unlikely]] {
        return;
    }

    const u32 end_offset = static_cast<u32>(
        std::min<std::size_t>(flush_end - surface.addr, mapped_size));
    if (start_offset >= end_offset) [[unlikely]] {
        return;
    }
