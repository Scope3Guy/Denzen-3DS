...
public:
    ...
    u32 session_id = 0;
    std::string url;
    RequestMethod method = RequestMethod::None;
    ...
    const ClCertAData* clcert_data = nullptr;
    ...
    PostDataType post_data_type = PostDataType::AsciiForm;
    std::string multipart_boundary;
    bool force_multipart = false;
    bool chunked_request = false;
    u32 chunked_content_length = 0;

    httplib::Response response;
    std::future<void> request_future;
    std::mutex response_mutex;
    std::condition_variable response_cv;
    std::deque<u8> response_body_buffer;
    std::atomic<u64> current_download_size_bytes{0};
    std::atomic<u64> total_download_size_bytes{0};
    u64 current_copied_data = 0;
    ...
    void MakeRequestSSL(httplib::Request& request, const URLInfo& url_info,
                        std::vector<Context::RequestHeader>& pending_headers);
    void ConfigureStreamingRequest(httplib::Request& request);
    bool HandleResponseHeaders(const httplib::Response& received_response);
    bool HandleResponseContent(const char* data, std::size_t data_length, u64 offset,
                               u64 total_length);
    void FinishRequest(bool request_success, httplib::Error error);
    void CancelRequest();
    Result WaitForResponseHeaders(bool timeout, u64 timeout_nanos);
    Result WaitForBodyData(std::size_t requested_size, bool timeout, u64 timeout_nanos);
    std::vector<u8> CopyBodyData(std::size_t max_size, Result& result);
...
private:
     ...
     *  Inputs:
     *      1 : Context handle
     *      2 : Proxy host string length
     *      3 : Proxy port
     *      4 : Proxy username string length
     *      5 : Proxy password string length
     *      6-11 : Proxy host, username, and password static buffers
     *  Outputs:
     *      1 : Result of function, 0 on success, otherwise error code
     */
    ...
    /// The next handle number to use when a new root certificate chain is created.
    RootCertChain::Handle root_cert_chain_counter = 0;

    /// The next handle number to use when a new root certificate is added to a chain.
    RootCertChain::RootCACert::Handle root_certs_counter = 0;
    ...
    /// Global list of root certificate chains currently opened.
