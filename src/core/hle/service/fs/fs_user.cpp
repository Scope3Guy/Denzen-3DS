...
void FS_USER::AddSeed(Kernel::HLERequestContext& ctx) {
    ...
    if (!FileSys::AddSeed({title_id, seed, {}})) {
        LOG_ERROR(Service_FS, "Failed to persist seed for title_id={:016X}", title_id);
        IPC::RequestBuilder rb{rp.MakeBuilder(1, 0)};
        rb.Push(ResultUnknown);
        return;
    }

    Service::AM::FinalizeSeededContentInstall(title_id);
