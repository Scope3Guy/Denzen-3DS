...
static std::string GetSystemSaveDataPath(u32 low) {
    const FileSys::Path archive_path = FileSys::ConstructSystemSaveDataBinaryPath(0, low);
    const std::string base_path =
        FileSys::GetSystemSaveDataContainerPath(FileUtil::GetUserPath(FileUtil::UserPath::NANDDir));
    return FileSys::GetSystemSaveDataPath(base_path, archive_path);
}

static std::string FindMissingFile(u32 save_id, std::initializer_list<std::string_view> files) {
    const std::string save_path = GetSystemSaveDataPath(save_id);
    for (std::string_view file : files) {
        const std::string path = save_path + std::string(file);
...
static std::string FindMissingOnlineModuleSystemSaveData(std::string_view module_name) {
    if (module_name == "ACT") {
        return FindMissingFile(0x0001002C, {"account.dat", "hash.dat", "initacc.dat"});
    }
    if (module_name == "BOSS") {
        return FindMissingFile(0x00010034, {"BOSS_SV.db", "BOSS_SS.db"});
    }
    if (module_name == "CECD") {
        return FindMissingFile(0x00010026, {"CEC/MBoxList____", "eventlog.dat"});
    }
    if (module_name == "FRD") {
        return FindMissingFile(0x00010032, {"config", "1/account"});
    }
    if (module_name == "NIM") {
        return FindMissingFile(0x00010038, {"persisid.dat", "transid.dat", "uuid.dat"});
    }
    return {};
}

static bool AttemptLLE(const ServiceModuleInfo& service_module, u64 loading_titleid) {
    const bool module_explicitly_enabled = Settings::values.lle_modules.at(service_module.name);
    ...
    if (!module_explicitly_enabled &&
        (!enable_recommended_lle_modules || !service_module.is_online_recommended))
        return false;

    if (!module_explicitly_enabled && service_module.is_online_recommended) {
        const std::string missing_file =
            FindMissingOnlineModuleSystemSaveData(service_module.name);
        if (!missing_file.empty()) {
            LOG_WARNING(Service,
                        "Service module \"{}\" requires complete console system save data for "
                        "safe LLE; missing \"{}\". Using Denzen HLE fallback.",
                        service_module.name, missing_file);
            return false;
        }
    }
