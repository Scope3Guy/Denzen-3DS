...
NCCHCryptoFile::NCCHCryptoFile(const std::string& out_file, bool use_console_unique_output) {
    if (use_console_unique_output) {
        ...
        if (!ncch_header.no_crypto) {
            ...
            if (ncch_header.fixed_key) {
                ...
                if (!ncch_header.seed_crypto) {
                    ...
                    if (!opt.has_value()) {
                        if (!allow_deferred_seed_decryption) {
                            LOG_ERROR(Service_AM, "Seed for program {:016X} not found",
                                      ncch_header.program_id);
                            is_error = true;
                        } else {
                            LOG_INFO(Service_AM,
                                     "Seed for program {:016X} not found; preserving encrypted "
                                     "NCCH for deferred seed import",
                                     ncch_header.program_id);
                            preserving_seed_encrypted_content = true;
                            is_encrypted = false;
                        }
                    } else {
                        auto seed{*opt};
                ...
                if (!preserving_seed_encrypted_content) {
...
void CIAFile::UsePortableDecryptedContentFromHLE() {
    use_console_unique_content_storage = false;
}

void CIAFile::AllowDeferredSeedDecryptionFromHLE() {
    allow_deferred_seed_decryption = true;
}

void AuthorizeServiceCIAInstallFromHLE(CIAFile& cia_file, const char* source) {
    cia_file.AuthorizeDecryptionFromHLE();
    cia_file.UsePortableDecryptedContentFromHLE();
    cia_file.AllowDeferredSeedDecryptionFromHLE();
    LOG_INFO(Service_AM, "Authorized service CIA install decryption for {}.", source);
}
    ...
    for (std::size_t i = 0; i < content_written.size(); i++) {
        if (content_written[i] < container.GetContentSize(i)) {
            ...
            if (i != current_content_index) {
                ...
                const bool use_console_unique_output =
                    decryption_authorized && use_console_unique_content_storage;
                current_content_file = std::make_unique<NCCHCryptoFile>(
                    content_file_paths[i], use_console_unique_output);
                current_content_file->decryption_authorized = decryption_authorized;
                current_content_file->allow_deferred_seed_decryption =
                    allow_deferred_seed_decryption;
...
Result CIAFile::ProvideTitleMetadata(std::span<const u8> tmd_data) {
    ASSERT_MSG(from_cdn, "This method should only be used when installing from CDN");
    ...
    InstallResult result = WriteTitleMetadata(tmd_data, 0);
    install_results.push_back(result);
    return result.result;
}
...
ResultVal<std::size_t> CIAFile::WriteContentDataIndexed(u16 content_index, u64 offset,
    ...
    if (content_index != current_content_index) {
        ...
        current_content_file = std::make_unique<NCCHCryptoFile>(content_file_paths[content_index],
                                                                use_console_unique_output);
        current_content_file->decryption_authorized = decryption_authorized;
        current_content_file->allow_deferred_seed_decryption = allow_deferred_seed_decryption;
...
InstallStatus InstallCIA(const std::string& path,
    ...
    if (container.Load(in_file.get()) == Loader::ResultStatus::Success) {
        ...
        const bool has_encrypted_content =
            container.GetTitleMetadata().HasEncryptedContent(container.GetHeader());
        if (has_encrypted_content) {
            if (!container.GetTicket().GetTitleKey()) {
                LOG_ERROR(Service_AM,
                          "File {} is encrypted and no title key is available! Aborting...", path);
                return InstallStatus::ErrorEncrypted;
            }

            installFile.AuthorizeDecryptionFromHLE();
            installFile.UsePortableDecryptedContentFromHLE();
        }
    ...
    if (container.Load(in_file.get()) == Loader::ResultStatus::Success) {
        ...
        if (check_encryption) {
            if (tmd.HasEncryptedContent(container.GetHeader())) {
                return container.GetTicket().GetTitleKey() ? InstallStatus::Success
...
InstallStatus InstallFromNus(u64 title_id, int version,
                             std::function<ProgressCallback>&& update_callback) {
    LOG_INFO(Service_AM, "Downloading {:016X} from NUS...", title_id);
    ...
    const auto tmd_response = Core::NUS::Download(path);
    if (!tmd_response) {
    ...
    if (tmd.Load(*tmd_response) != Loader::ResultStatus::Success || tmd.GetTitleID() != title_id) {
        LOG_ERROR(Service_AM, "Downloaded invalid TMD for {:016X}", title_id);
        return InstallStatus::ErrorInvalid;
    }

    path = fmt::format("/ccs/download/{:016X}/cetk", title_id);
    const auto cetk_response = Core::NUS::Download(path);
    if (!cetk_response) {
        LOG_ERROR(Service_AM, "Failed to download ticket for {:016X}", title_id);
        return InstallStatus::ErrorFileNotFound;
    }

    FileSys::Ticket ticket;
    if (ticket.Load(*cetk_response) != Loader::ResultStatus::Success ||
        ticket.GetTitleID() != title_id) {
        LOG_ERROR(Service_AM, "Downloaded invalid ticket for {:016X}", title_id);
        return InstallStatus::ErrorInvalid;
    }

    const bool has_encrypted_content = tmd.HasEncryptedContent();
    if (has_encrypted_content && !ticket.GetTitleKey()) {
        LOG_ERROR(Service_AM, "No title key is available for encrypted NUS title {:016X}", title_id);
        return InstallStatus::ErrorEncrypted;
    }
    ...
    for (std::size_t i = 0; i < content_count; ++i) {
        total_content_size += tmd.GetContentSizeByIndex(i);
    }
    ...
    FileSys::CIAHeader fake_header{
        ...
        .meta_size = 0,
        .content_size = static_cast<u64_le>(total_content_size),
    };
    for (std::size_t i = 0; i < content_count; ++i) {
        // CIA headers are indexed by TMD content index, but this container also uses
        // positional indexes in existing install paths.
        fake_header.SetContentPresent(static_cast<u16>(i));
        fake_header.SetContentPresent(tmd.GetContentIndexByIndex(i));
    }

    std::vector<u8> header_data(sizeof(fake_header));
    std::memcpy(header_data.data(), &fake_header, sizeof(fake_header));

    Service::AM::CIAFile install_file(Core::System::GetInstance(), GetTitleMediaType(title_id));
    if (has_encrypted_content) {
        install_file.AuthorizeDecryptionFromHLE();
        install_file.UsePortableDecryptedContentFromHLE();
    }

    std::size_t current_offset = 0;
    const auto write_bytes = [&install_file, &current_offset](std::span<const u8> bytes,
                                                              const char* section_name) {
        if (bytes.empty()) {
            return InstallStatus::Success;
        }

        const auto result = install_file.Write(static_cast<u64>(current_offset), bytes.size(), true,
                                               false, bytes.data());
        if (result.Failed()) {
            LOG_ERROR(Service_AM, "Failed to install NUS {} at offset {}: {:08x}", section_name,
                      current_offset, result.Code().raw);
            return InstallStatus::ErrorAborted;
        }
        if (*result != bytes.size()) {
            LOG_ERROR(Service_AM, "Short write while installing NUS {} at offset {}: {} of {}",
                      section_name, current_offset, *result, bytes.size());
            return InstallStatus::ErrorAborted;
        }
    ...
    const auto align_stream = [&write_bytes, &current_offset](const char* section_name) {
        const auto aligned_offset = Common::AlignUp(current_offset, FileSys::CIA_SECTION_ALIGNMENT);
        std::size_t padding_size = aligned_offset - current_offset;
        std::array<u8, FileSys::CIA_SECTION_ALIGNMENT> padding{};

        while (padding_size > 0) {
            const std::size_t chunk_size = std::min(padding_size, padding.size());
            const auto status = write_bytes(std::span<const u8>{padding.data(), chunk_size},
                                            section_name);
    ...
    const auto write_section_aligned = [&write_bytes, &align_stream](std::span<const u8> bytes,
                                                                     const char* section_name) {
        const auto write_status = write_bytes(bytes, section_name);
        if (write_status != InstallStatus::Success) {
            return write_status;
        }
        return align_stream(section_name);
    };

    auto status = write_section_aligned(std::span<const u8>{header_data.data(), header_data.size()},
                                        "CIA header");
    if (status != InstallStatus::Success) {
        return status;
    }

    status = write_section_aligned(std::span<const u8>{cetk_response->data(), cetk_response->size()},
                                   "ticket");
    if (status != InstallStatus::Success) {
        return status;
    }

    status = write_section_aligned(std::span<const u8>{tmd_response->data(), tmd_response->size()},
                                   "TMD");
    if (status != InstallStatus::Success) {
        return status;
    }
    ...
    const auto total_progress_size = static_cast<std::size_t>(total_content_size);
    if (update_callback) {
        update_callback(total_bytes_written, total_progress_size);
    }
    ...
    for (std::size_t i = 0; i < content_count; ++i) {
        ...
        const auto content_response = Core::NUS::Download(path);
        if (!content_response) {
            LOG_ERROR(Service_AM, "Failed to download content {} for {:016X}", filename, title_id);
            return InstallStatus::ErrorFileNotFound;
        }

        const auto expected_size = tmd.GetContentSizeByIndex(i);
        if (content_response->size() != expected_size) {
            LOG_ERROR(Service_AM,
                      "Downloaded content {} for {:016X} has size {}, expected {}", filename,
                      title_id, content_response->size(), expected_size);
            return InstallStatus::ErrorInvalid;
        }

        status = write_bytes(std::span<const u8>{content_response->data(), content_response->size()},
                             filename.c_str());
        if (status != InstallStatus::Success) {
            return status;
        }

        total_bytes_written += content_response->size();
        if (update_callback) {
            update_callback(total_bytes_written, total_progress_size);
        }
    }
    ...
    for (const auto& result : install_file.GetInstallResults()) {
        if (result.result.IsError()) {
            LOG_ERROR(Service_AM, "NUS title {:016X} installation produced error {:08x}", title_id,
                      result.result.raw);
            return InstallStatus::ErrorAborted;
        }
    }

    LOG_INFO(Service_AM, "Installed NUS title {:016X} successfully.", title_id);
    return InstallStatus::Success;
}
...
bool FinalizeSeededContentFile(const std::string& content_path, u64 title_id) {
    if (!FileUtil::Exists(content_path)) {
        return false;
    }

    std::unique_ptr<FileUtil::IOFile> input = std::make_unique<FileUtil::IOFile>(content_path, "rb");
    if (!input->IsOpen()) {
        LOG_ERROR(Service_AM, "Failed to open seed-crypto content for finalization: {}",
                  content_path);
        return false;
    }

    if (FileUtil::Z3DSReadIOFile::GetUnderlyingFileMagic(input.get()) != std::nullopt) {
        input = std::make_unique<FileUtil::Z3DSReadIOFile>(std::move(input));
    }

    NCCH_Header header{};
    if (input->ReadBytes(&header, sizeof(header)) != sizeof(header)) {
        LOG_ERROR(Service_AM, "Failed to read NCCH header for seed finalization: {}",
                  content_path);
        return false;
    }

    if (header.magic != Loader::MakeMagic('N', 'C', 'C', 'H') || header.no_crypto ||
        !header.seed_crypto || header.program_id != title_id) {
        return false;
    }

    if (!FileSys::GetSeed(title_id).has_value()) {
        LOG_WARNING(Service_AM,
                    "Cannot finalize seed-crypto content for title_id={:016X}; seed is missing",
                    title_id);
        return false;
    }

    if (!input->Seek(0, SEEK_SET)) {
        LOG_ERROR(Service_AM, "Failed to rewind seed-crypto content for finalization: {}",
                  content_path);
        return false;
    }

    const std::string temp_path = content_path + ".seedtmp";
    const std::string backup_path = content_path + ".seedbak";
    FileUtil::Delete(temp_path);
    FileUtil::Delete(backup_path);

    {
        NCCHCryptoFile output(temp_path, false);
        output.AuthorizeDecryptionFromHLE();

        const u64 input_size = input->GetSize();
        u64 total_read = 0;
        std::vector<u8> buffer(0x10000);
        while (total_read < input_size) {
            const std::size_t bytes_to_read = static_cast<std::size_t>(
                std::min<u64>(static_cast<u64>(buffer.size()), input_size - total_read));
            const std::size_t bytes_read = input->ReadBytes(buffer.data(), bytes_to_read);
            if (bytes_read != bytes_to_read) {
                LOG_ERROR(Service_AM,
                          "Short read while finalizing seed-crypto content: {} read={} expected={}",
                          content_path, bytes_read, bytes_to_read);
                FileUtil::Delete(temp_path);
                return false;
            }
            ...
            if (output.IsError()) {
                LOG_ERROR(Service_AM, "Failed to decrypt seed-crypto content for title_id={:016X}",
                          title_id);
                FileUtil::Delete(temp_path);
                return false;
            }
    ...
    FileSys::NCCHContainer finalized(temp_path);
    if (finalized.LoadHeader() != Loader::ResultStatus::Success) {
        LOG_ERROR(Service_AM, "Finalized seed-crypto content failed NCCH validation: {}",
                  temp_path);
        FileUtil::Delete(temp_path);
        return false;
    }

    if (!FileUtil::Rename(content_path, backup_path)) {
        LOG_ERROR(Service_AM, "Failed to backup seed-crypto content before finalization: {}",
                  content_path);
        FileUtil::Delete(temp_path);
        return false;
    }

    if (!FileUtil::Rename(temp_path, content_path)) {
        LOG_ERROR(Service_AM, "Failed to replace seed-crypto content after finalization: {}",
                  content_path);
        FileUtil::Rename(backup_path, content_path);
        FileUtil::Delete(temp_path);
        return false;
    }

    FileUtil::Delete(backup_path);
    LOG_INFO(Service_AM, "Finalized seed-crypto content for title_id={:016X}: {}", title_id,
             content_path);
    return true;
}

bool FinalizeSeededContentInstall(u64 title_id) {
    const std::array<Service::FS::MediaType, 2> media_types = {Service::FS::MediaType::NAND,
                                                              Service::FS::MediaType::SDMC};
    bool finalized_any = false;

    for (const auto media_type : media_types) {
        const std::string tmd_path = GetTitleMetadataPath(media_type, title_id);
        ...
        if (tmd.Load(tmd_path) != Loader::ResultStatus::Success) {
            LOG_WARNING(Service_AM,
                        "Unable to load TMD while finalizing seeded content title_id={:016X}, media_type={}",
                        title_id, media_type);
            continue;
        }
        ...
        for (std::size_t i = 0; i < tmd.GetContentCount(); ++i) {
            finalized_any |=
                FinalizeSeededContentFile(GetTitleContentPath(media_type, title_id, i), title_id);
        }
    }

    if (!finalized_any) {
        LOG_DEBUG(Service_AM, "No deferred seed-crypto content needed finalization for title_id={:016X}",
                  title_id);
    }
...
void Module::Interface::GetTwlArchiveResourceInfo(Kernel::HLERequestContext& ctx) {
    IPC::RequestParser rp(ctx);

    // AM_TWLPartitionInfo in libctru is four u64 values:
    // capacity, freeSpace, titlesCapacity, titlesFreeSpace.
    constexpr u64 TwlNandCapacity = 128ULL * 1024ULL * 1024ULL;
    constexpr u64 TwlNandFreeSpace = 96ULL * 1024ULL * 1024ULL;
    constexpr u64 TwlTitlesCapacity = TwlNandCapacity;
    constexpr u64 TwlTitlesFreeSpace = TwlNandFreeSpace;

    IPC::RequestBuilder rb = rp.MakeBuilder(9, 0);
    rb.Push(ResultSuccess);
    rb.Push<u64>(TwlNandCapacity);
    rb.Push<u64>(TwlNandFreeSpace);
    rb.Push<u64>(TwlTitlesCapacity);
    rb.Push<u64>(TwlTitlesFreeSpace);
...
void Module::Interface::BeginImportProgram(Kernel::HLERequestContext& ctx) {
    ...
    std::shared_ptr<Service::FS::File> file;
    {
        auto cia_file = std::make_unique<CIAFile>(am->system, media_type);
        AuthorizeServiceCIAInstallFromHLE(*cia_file, "AM::BeginImportProgram");
        file =
...
void Module::Interface::BeginImportProgramTemporarily(Kernel::HLERequestContext& ctx) {
    ...
    std::shared_ptr<Service::FS::File> file;
    {
        ...
        AuthorizeServiceCIAInstallFromHLE(*cia_file, "AM::BeginImportProgramTemporarily");
...
void Module::Interface::BeginImportTitle(Kernel::HLERequestContext& ctx) {
    ...
    AuthorizeServiceCIAInstallFromHLE(am->importing_title->cia_file, "AM::BeginImportTitle");
