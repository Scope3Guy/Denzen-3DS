...
public:
    NCCHCryptoFile(const std::string& out_file, bool use_console_unique_output);

    void Write(const u8* buffer, std::size_t length);
    void AuthorizeDecryptionFromHLE() {
        decryption_authorized = true;
    }
    void AllowDeferredSeedDecryptionFromHLE() {
...
private:
    ...
    bool allow_deferred_seed_decryption = false;
    bool preserving_seed_encrypted_content = false;
...
public:
    ...
    Result ProvideTitleMetadata(std::span<const u8> tmd_data);
    ...
    void AuthorizeDecryptionFromHLE();
    // Frontend and official-system installers can decrypt with title keys without requiring
    // console-unique data.
    void UsePortableDecryptedContentFromHLE();
    void AllowDeferredSeedDecryptionFromHLE();
...
private:
    ...
    bool use_console_unique_content_storage = true;
    bool allow_deferred_seed_decryption = false;
    bool is_done = false;
...
/**
 * Downloads a title from Nintendo Update Servers and installs it directly.
 * @param title_id title ID to download
 * @param version optional title version, or -1 for latest
 * @param update_callback callback function called during content writes
 */
...
/**
 * Finalize any installed seed-crypto content for a title after FS imports its seed.
 */
...
public:
    ...
    protected:
        ...
        /**
         * AM::GetTwlArchiveResourceInfo service function
         * Outputs:
         *      1 : Result of function, 0 on success, otherwise error code
         *    2-3 : TWL NAND capacity
         *    4-5 : TWL NAND free space
         *    6-7 : TWL title capacity
         *    8-9 : TWL title free space
         */
        void GetTwlArchiveResourceInfo(Kernel::HLERequestContext& ctx);
        /**
