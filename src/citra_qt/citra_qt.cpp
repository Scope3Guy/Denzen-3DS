...
static QString GetSystemTitleRegionName(u32 region) {
    static constexpr std::array<const char*, Core::NUM_SYSTEM_TITLE_REGIONS> region_names{
        "JPN", "USA", "EUR", "AUS", "CHN", "KOR", "TWN",
    };

    if (region >= region_names.size()) {
        return QString{};
    }

    return QString::fromLatin1(region_names[region]);
}

static QString InstallStatusToString(Service::AM::InstallStatus status) {
    switch (status) {
    case Service::AM::InstallStatus::Success:
        return QStringLiteral("success");
    case Service::AM::InstallStatus::ErrorFailedToOpenFile:
        return QStringLiteral("failed to open file");
    case Service::AM::InstallStatus::ErrorFileNotFound:
        return QStringLiteral("downloaded file not found");
    case Service::AM::InstallStatus::ErrorAborted:
        return QStringLiteral("installation aborted");
    case Service::AM::InstallStatus::ErrorInvalid:
        return QStringLiteral("invalid downloaded title data");
    case Service::AM::InstallStatus::ErrorEncrypted:
        return QStringLiteral("encrypted content could not be installed");
    }
...
void GMainWindow::OnMenuSetUpSystemFiles() {
    ...
    QLabel label_description(
        tr("Install decrypted CIA or ZCIA files dumped from your console, or use the Artic Setup "
           "Tool if you want to copy console-unique data from a live console."),
        &dialog);
    label_description.setWordWrap(true);
    layout.addWidget(&label_description);

    QGroupBox local_setup_group(tr("Install from CIA files"), &dialog);
    QVBoxLayout local_setup_layout(&local_setup_group);

    QLabel local_setup_description(
        tr("Use this for dumped system titles, applets, fonts, archives, updates, and other CIA "
           "files. This path uses the normal CIA installer and does not require Artic Base."),
        &local_setup_group);
    local_setup_description.setWordWrap(true);
    local_setup_layout.addWidget(&local_setup_description);

    QPushButton install_cia_button(tr("Install CIA Files..."), &local_setup_group);
    local_setup_layout.addWidget(&install_cia_button, 0, Qt::AlignLeft);
    layout.addWidget(&local_setup_group);

    QGroupBox download_setup_group(tr("Download official system files"), &dialog);
    QVBoxLayout download_setup_layout(&download_setup_group);

    QLabel download_setup_description(
        tr("Download the latest system titles for a selected region from Nintendo's update "
           "servers and install them to NAND. This does not install console-unique data."),
        &download_setup_group);
    download_setup_description.setWordWrap(true);
    download_setup_layout.addWidget(&download_setup_description);

    auto* download_controls_layout = new QGridLayout;
    download_setup_layout.addLayout(download_controls_layout);

    QLabel download_model_label(tr("Model:"), &download_setup_group);
    download_controls_layout->addWidget(&download_model_label, 0, 0);

    QComboBox download_model_combo(&download_setup_group);
    download_model_combo.addItem(tr("Old 3DS"), static_cast<u32>(Core::SystemTitleSet::Old3ds));
    download_model_combo.addItem(tr("New 3DS"), static_cast<u32>(Core::SystemTitleSet::New3ds));
    download_model_combo.setCurrentIndex(Settings::values.is_new_3ds.GetValue() ? 1 : 0);
    download_controls_layout->addWidget(&download_model_combo, 0, 1);

    QLabel download_region_label(tr("Region:"), &download_setup_group);
    download_controls_layout->addWidget(&download_region_label, 1, 0);

    QComboBox download_region_combo(&download_setup_group);
    for (u32 region = 0; region < Core::NUM_SYSTEM_TITLE_REGIONS; ++region) {
        download_region_combo.addItem(GetSystemTitleRegionName(region), region);
    }

    const s32 configured_region = Settings::values.region_value.GetValue();
    const u32 default_download_region =
        configured_region >= 0 && configured_region < static_cast<s32>(Core::NUM_SYSTEM_TITLE_REGIONS)
            ? static_cast<u32>(configured_region)
            : 1;
    download_region_combo.setCurrentIndex(static_cast<int>(default_download_region));
    download_controls_layout->addWidget(&download_region_combo, 1, 1);

    QPushButton download_system_files_button(tr("Download and Install"), &download_setup_group);
    download_controls_layout->addWidget(&download_system_files_button, 0, 2, 2, 1);
    download_controls_layout->setColumnStretch(1, 1);
    layout.addWidget(&download_setup_group);

    QGroupBox artic_setup_group(tr("Set up from Artic Setup Tool"), &dialog);
    QVBoxLayout artic_layout(&artic_setup_group);
    ...
    QLabel artic_description(
        tr("<p>Use this only when you can run the setup tool on a real console.<br><a "
           ...
           "model of the console running the setup tool.</li></ul></p>"),
        &artic_setup_group);
    artic_description.setOpenExternalLinks(true);
    artic_layout.addWidget(&artic_description);
    ...
    QLineEdit textInput(UISettings::values.last_artic_base_addr, &artic_setup_group);
    layout_h->addWidget(&textInput);

    QLabel label_select(tr("<br>Choose setup mode:"), &artic_setup_group);
    artic_layout.addWidget(&label_select);

    std::pair<bool, bool> install_state = Core::AreSystemTitlesInstalled();

    QRadioButton radio1(&artic_setup_group);
    QRadioButton radio2(&artic_setup_group);
    ...
    artic_layout.addWidget(&radio1);
    artic_layout.addWidget(&radio2);
    layout.addWidget(&artic_setup_group);

    QDialogButtonBox buttonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, &dialog);
    if (auto* connect_button = buttonBox.button(QDialogButtonBox::Ok)) {
    ...
    bool install_cia_files = false;
    connect(&install_cia_button, &QPushButton::clicked, &dialog, [&] {
        install_cia_files = true;
        dialog.accept();
    });

    bool download_system_files = false;
    Core::SystemTitleSet download_system_title_set = Core::SystemTitleSet::New3ds;
    u32 download_region = 0;
    connect(&download_system_files_button, &QPushButton::clicked, &dialog, [&] {
        download_system_files = true;
        download_system_title_set = static_cast<Core::SystemTitleSet>(
            download_model_combo.currentData().toUInt());
        download_region = download_region_combo.currentData().toUInt();
        dialog.accept();
    });

    int res = dialog.exec();
    if (res == QDialog::Accepted) {
        if (install_cia_files) {
            InstallCIAFromFileDialog(tr("Install System CIA Files"));
            return;
        }

        if (download_system_files) {
            DownloadSystemFiles(download_system_title_set, download_region);
            return;
        }
...
void GMainWindow::OnMenuInstallCIA() {
    InstallCIAFromFileDialog(tr("Load Files"));
}

void GMainWindow::InstallCIAFromFileDialog(const QString& dialog_title) {
    QStringList filepaths = QFileDialog::getOpenFileNames(
        this, dialog_title, UISettings::values.roms_path,
...
void GMainWindow::DownloadSystemFiles(Core::SystemTitleSet system_title_set, u32 region) {
    if (region >= Core::NUM_SYSTEM_TITLE_REGIONS) {
        return;
    }

    const QString region_name = GetSystemTitleRegionName(region);
    const QString model_name = system_title_set == Core::SystemTitleSet::New3ds ? tr("New 3DS")
                                                                                : tr("Old 3DS");
    const std::vector<u64> title_ids = Core::GetSystemTitleIds(system_title_set, region);
    if (title_ids.empty()) {
        QMessageBox::information(this, tr("Download System Files"),
                                 tr("No %1 system titles are known for %2.")
                                     .arg(model_name)
                                     .arg(region_name));
        return;
    }
    ...
    const auto answer = QMessageBox::question(
        this, tr("Download System Files"),
        tr("Download and install %1 %2 system titles for %3 from Nintendo's update servers?\n\n"
           "This installs system titles only. Console-unique data still requires a local dump or "
           "Artic setup.")
            .arg(static_cast<qulonglong>(title_ids.size()))
            .arg(model_name)
            .arg(region_name),
        QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    if (answer != QMessageBox::Yes) {
    ...
    ui->action_Setup_System_Files->setEnabled(false);
    ui->action_Install_CIA->setEnabled(false);
    SCOPE_EXIT({
        game_list->SetDirectoryWatcherEnabled(true);
        ui->action_Setup_System_Files->setEnabled(true);
        ui->action_Install_CIA->setEnabled(true);
    });

    const int progress_max = static_cast<int>(title_ids.size() * 100);
    QProgressDialog progress(tr("Downloading system files..."), tr("Cancel"), 0, progress_max,
                             this);
    ...
    Service::AM::InstallStatus failed_status = Service::AM::InstallStatus::Success;
    u64 failed_title_id = 0;
    std::size_t completed_titles = 0;

    connect(&progress, &QProgressDialog::canceled, &progress, [&cancelled] {
        cancelled = true;
    });
    connect(&future_watcher, &QFutureWatcher<void>::finished, &progress, &QProgressDialog::reset);

    const auto future = QtConcurrent::run([this, title_ids, region_name, model_name, progress_ptr,
                                           &cancelled, &failed_status, &failed_title_id,
                                           &completed_titles] {
        const std::size_t total_titles = title_ids.size();
        for (std::size_t index = 0; index < total_titles; ++index) {
            if (cancelled) {
                return;
            }

            const u64 title_id = title_ids[index];
            if (progress_ptr) {
                QMetaObject::invokeMethod(
                    progress_ptr.data(),
                    [this, progress_ptr, region_name, model_name, index, total_titles, title_id] {
                        if (!progress_ptr) {
                            return;
                        }
                        progress_ptr->setLabelText(
                            tr("Downloading %1 %2 system files...\nTitle %3 of %4: %5")
                                .arg(model_name)
                                .arg(region_name)
                                .arg(static_cast<qulonglong>(index + 1))
                                .arg(static_cast<qulonglong>(total_titles))
                                .arg(QString::number(static_cast<qulonglong>(title_id), 16)
                                         .toUpper()
            ...
            const auto status = Service::AM::InstallFromNus(
                title_id, -1,
                [progress_ptr, index](std::size_t written, std::size_t total) {
                    if (!progress_ptr || total == 0) {
                        return;
                    }

                    const int value = static_cast<int>(index * 100 + (written * 100 / total));
                    QMetaObject::invokeMethod(
                        progress_ptr.data(),
                        [progress_ptr, value] {
                            if (progress_ptr) {
                                progress_ptr->setValue(value);
                            }
                        },
                        Qt::QueuedConnection);
                });
            ...
            if (progress_ptr) {
                QMetaObject::invokeMethod(
                    ...
                    [progress_ptr, index] {
                        if (progress_ptr) {
                            progress_ptr->setValue(static_cast<int>((index + 1) * 100));
                        }
                    },
                    Qt::QueuedConnection);
            }
    ...
    if (failed_status != Service::AM::InstallStatus::Success) {
        QMessageBox::critical(
            this, tr("Download System Files"),
            tr("Failed to download or install title %1.\n\nReason: %2")
                .arg(QString::number(static_cast<qulonglong>(failed_title_id), 16)
                         .toUpper()
                         .rightJustified(16, QLatin1Char('0')))
                .arg(InstallStatusToString(failed_status)));
    } else if (completed_titles < title_ids.size()) {
        QMessageBox::information(this, tr("Download System Files"),
                                 tr("System file download was canceled."));
    } else {
        QMessageBox::information(
            this, tr("Download System Files"),
            tr("Successfully downloaded and installed %1 %2 system titles for %3.")
                .arg(static_cast<qulonglong>(title_ids.size()))
                .arg(model_name)
                .arg(region_name));
    }
